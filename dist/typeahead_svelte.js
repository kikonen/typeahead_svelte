(function (internal, svelte) {
  ...
  const Typeahead = internal.create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
  	...
  	if ($$props.real === void 0 && $$bindings.real && real !== void 0) $$bindings.real(real);
  	if ($$props.fetcher === void 0 && $$bindings.fetcher && fetcher !== void 0) $$bindings.fetcher(fetcher);
  	if ($$props.queryMinLen === void 0 && $$bindings.queryMinLen && queryMinLen !== void 0) $$bindings.queryMinLen(queryMinLen);
  	if ($$props.onSelected === void 0 && $$bindings.onSelected && onSelected !== void 0) $$bindings.onSelected(onSelected);
  	$$result.css.add(css);
...
<div class="${"input-group mb-3 typeahead js-typeahead-container svelte-2ap05h"}">
  <input class="${"js-input " + internal.escape(real.getAttribute("class")) + " svelte-2ap05h"}"${internal.add_attribute("data-target", real.id, 0)}${internal.add_attribute("placeholder", real.placeholder, 0)}${internal.add_attribute("this", input, 1)}${internal.add_attribute("value", query, 1)}>
  <div class="${"input-group-append"}">
    <button class="${"btn btn-outline-secondary"}" type="${"button"}" tabindex="${"-1"}"${internal.add_attribute("this", toggle, 1)}>
      <i class="${"fas fa-caret-down"}"></i>
    </button>
  </div>

  <div class="${"js-popup dropdown-menu typeahead-popup " + internal.escape(popupVisible ? "show" : "") + " svelte-2ap05h"}"${internal.add_attribute("this", popup, 1)}>
    ${fetchError
	? `<div tabindex="${"-1"}" class="${"dropdown-item text-danger"}">
        ${internal.escape(fetchError)}
      </div>`
	: `${activeFetch
		? `${!fetchingMore
			? `<div tabindex="${"-1"}" class="${"dropdown-item text-muted"}">
            ${internal.escape(i18n.fetching)}
          </div>`
			: ``}`
		: `${entries.length === 0
			? `<div tabindex="${"-1"}" class="${"dropdown-item text-muted"}">
            ${tooShort
				? `${internal.escape(i18n.too_short)}`
				: `${internal.escape(i18n.no_results)}`}
          </div>`
			: ``}`}`}

    ${(!activeFetch || fetchingMore) && entries.length > 0
	? `${internal.each(entries, (item, index) => `<div tabindex="${"1"}" class="${"js-item dropdown-item"}"${internal.add_attribute("data-index", index, 0)}>
          <div class="${"no-click svelte-2ap05h"}">
            ${internal.escape(item.text)}
          </div>
          <div class="${"no-click text-muted svelte-2ap05h"}">
            ${internal.escape(item.desc)}
          </div>
        </div>`)}`
	: ``}

    ${hasMore
	? `<div tabindex="${"-1"}" class="${"js-more dropdown-item text-muted"}"${internal.add_attribute("this", more, 1)}>
        ${internal.escape(i18n.has_more)}
      </div>`
	: ``}
  </div>
</div>`;
